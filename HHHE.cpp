...
int main() {
    ...
    if (strcmp(player.name, "wzc") == 0) {
        SlowDisplay(
            "封印多年的 wzc 啊!你终于可以重见天日了!\n\n\n wzc 重新启动...      随即...     暴走!!!\n\n\n");
        player.attack = 999;
        player.defense = 999;
        player.health = 9999;
...
{
    printf(
        "物品: 1,止血草%d个 2,急救包%d个 3,云南白药%d个 4,超级云南白药%d个 5,手雷%d个 6,毒标%d个 "
        "7,手抛式原子弹%d个 0,返回\n\n\n",
    ...
    while (1) {
        ...
        switch (scanf("%d", &choose_number), choose_number) {
            case 1:  //显示移动菜单
                SlowDisplay("要去哪里?\n\n\n");
                printf(
                    "1,酒吧 2,旅馆 3,森林一层 4,森林二层 5,森林三层 6,草原一层 7,草原二层 8,草原三层\n\n\n");
                switch (scanf("%d", &choose_number), choose_number) {
                    case 1:
                    ...
                    case 8:
                        ...
                        if (s < 7) {
                            ...
                            if (strongman_arm) {
                                SlowDisplay(
                                    "神秘老人:\n\n\n    "
                                    "哈哈,年轻人,做的不错,不过...嘿嘿,你上当啦!巨人之臂我要了,"
                                    "公主你也别想带走!\n\n\n");
                                guai = shitu;
                                printf("%s扑了过来!\n\n\n", guai.name);
                                BattleAct();
            ...
            case 3:     //对话选项
                if (place_sign == place.bar) {
                    ...
                    switch (scanf("%d", &choose_number), choose_number) {
                        ...
                        case 3:
                            printf(
                                "要喝点什么?\n\n\n    1,二锅头25金币 HP+20  2,XO酒80金币 HP+50  "
                                "3,人头马面150金币 HP+100  0,返回\n\n\n");
                            choose_number = 1;
                            while (choose_number) {
                                switch (scanf("%d", &choose_number), choose_number) {
                                    case 1:
...
void DisplayState() {
    printf("%s 攻击力:%d+%d=%d  防御力:%d+%d=%d  HP:%d/%d \n\n\n", player.name, player.attack, gong,
           player.attack + gong, player.defense, fang, player.defense + fang, player.health,
           player.max_health);
    printf("武器: %s 防具: %s \n\n\n", gongname, fangname);
    printf("等级:%d  经验:%d/%d 还需要%d经验升级 金币:%d \n\n\n", player.level, player.exp, player.max_exp,
           player.max_exp - player.exp, money);
}
void BattleAct() {
    while (1) {
        puts("=============================================================================");
